...
rename, delete-via-plonk, delete-via-git, sweep, and prune, run against a real
temporary repository. It asserts that for any branch exactly one of the three
states holds — no record, a live record, or a tombstone — and that the subset
invariant still holds after every step.
