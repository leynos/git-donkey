...
`make lint` runs these Python lint checks in order:

1. Ruff for fast source and docstring-style checks.
2. `interrogate` for docstring coverage.
3. `pyscn` for static analysis.
4. The built-in Pylint pass for its curated upstream message set.
5. The df12-python-lints Pylint pass for the plugin's house-style checkers.
6. `ambrleaks` for unredacted values in syrupy snapshot files.
7. Skylos for strict production dead-code detection.
